...
#'
#' @export
#' @param sum.prop the estimated proportion of each component as a vector, posterior median or mean
...
calc_posteriors <- function(sum.prop, dat, alphas){
  B.dat <- dat$B
  SESQ.dat <- dat$SESQ
  ...
  posteriors <- lapply(1:N, function(i)
    .calc_posterior_variant_M4(B.dat[i,], SESQ.dat[i,], p, scalesq_vector, alphas))
...
.calc_posterior_variant_M4 <- function(B, SESQ, p, scalesq, alphas){
  zeros <- c(0,0)
  SESQ_mat <- matrix(c(SESQ[1], 0, 0, SESQ[2]), 2, 2)
  p_0 = p[1]*mnormt::dmnorm(B, zeros, SESQ_mat)
  p_1 = p[2]*mnormt::dmnorm(B, zeros, SESQ_mat + 
                              matrix(c(scalesq[2], scalesq[2] * alphas[2], scalesq[2] * alphas[2], scalesq[2] * alphas[2]^2),2,2))
  p_2 = p[3]*mnormt::dmnorm(B, zeros, SESQ_mat +
                              matrix(c(scalesq[3], scalesq[3] * alphas[3], scalesq[3] * alphas[3], scalesq[3] * alphas[3]^2),2,2))
  p_3 = p[4]*mnormt::dmnorm(B, zeros, SESQ_mat +
                              matrix(c(scalesq[4], scalesq[4] * alphas[4], scalesq[4] * alphas[4], scalesq[4] * alphas[4]^2),2,2))
